...
rowOrder = ["#first", "#second", "#third", "#fourth" ];
...
function startRow(){
intervalID = setInterval(selectNextRow,1500);
}
...
function startButton(){
	intervalID = setInterval(selectNext,1500);
}
...
function selectNextRow(){
	...
	selectedRow = getSelectedRowItem();
	if(selectedRow == null)
		selectRowItem(rowOrder[0]);
	else{

		indexRow = rowOrder.indexOf(selectedRow);
		indexRow = (indexRow + 1) % rowOrder.length;
		selectRowItem(rowOrder[indexRow])
	}

}

function getfirstButtonOfRow(name){
var button_val;
if(name == "first")
		button_val = buttonOrder[0];
else if (name == "second")
	button_val = buttonOrder[4];	
else if (name == "three")
	button_val = buttonOrder[8];	
else if (name == "fourth")
	button_val = buttonOrder[12];	
...
function selectNext() {
	...
	if (selected ==  null) {
		...
		var rowID = selectRow.first().attr('id');
			
		selectItem(getfirstButtonOfRow(rowID));
